...
(function($){
	...
	if ($("#dataTable_activity_admin").length) {
        ...
        if ($("#dataTable_activity_admin1").length) {
		var columnDefs = [
		  {
			"targets": 0,
			"createdCell": function (td, cellData, rowData, row, col) {
		...
		renderDataTable('dataTable_activity_admin1', 'query/users_otp_log/', columnDefs);
	}
	...
	if ($("#dataTable_user_ratings").length) {
		var status;
		var columnDefs = [
		  {
		    "targets": 0,
		...
		var full_url = 'query/users_ratings/' + $('#user_ids').val() + '/';
	...
	if ($("#dataTable_list_menu").length) {
		...
		var full_url = 'query/menusdata/' + $('#user_ids').val() + '/';
		renderDataTable('dataTable_list_menu', full_url, columnDefs);
	}
	...
	if ($("#blog_body").length) {
		$('#blog_body').summernote({height:350});
		$('#blog_body').summernote('code', $('#blog_body_value').val());
	}
	
	if ($("#blog_body1").length) {
		$('#blog_body1').summernote({height:350});
		$('#blog_body1').summernote('code', $('#blog_body_value1').val());
	}
	
	if ($("#documentation_body").length) {
		$('#documentation_body').summernote({height:350});
		$('#documentation_body').summernote('code', $('#documentation_body1').val());
	}
...
function renderDataTableButtonAppend(ids, type, append_data='') {  //handle special button group
	...
	switch (type) {
		case 'payment_subscription_admin' :
		  ...
		  else if (append_data == 'pending_cancellation') {
