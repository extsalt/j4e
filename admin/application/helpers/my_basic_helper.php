...
function checkpermissions($parent_id)
{
    $CI = &get_instance();
    $query_user = $CI->db->where('ids', $_SESSION['user_ids'])->get('user', 1)->row();
    $query_role = $CI->db->where('ids', $query_user->role_ids)->get('role', 1)->row();
    return $CI->db->get_where('module_permission',array('menu_id'=>$parent_id,'role_id'=>$query_user->role_ids))->row_array();
}
...
if (!function_exists('my_send_email')) {
	function my_send_email($email) {
		...
		if ($email_setting_array['host'] != '') {
			...
			$CI->email->message($email['email_body']);  //echo $email['email_body'];
			$CI->email->set_newline("\r\n");
			try {
				$send_result = $CI->email->send();         //echo $CI->email->print_debugger();   
			}
...
if (!function_exists('my_otp_log')) {
	function my_otp_log($user_ids, $level, $event, $detail, $debug_detail = '',$dt = '') {
		$CI = &get_instance();
		$insert_data = array(
		  'user_ids' => $user_ids,
		  'level' => $level,
		  ...
		  'debug_detail' => $debug_detail,
                ...
                if(!empty($dt))
                    {
                        $insert_data['created_time'] = $dt;
                    }
                    else
                    {
                        $insert_data['created_time'] = my_server_time();		
                    }
		$CI->db->insert('otp_logs', $insert_data);
		return TRUE;
	}
}
...
if (!function_exists('validate_post_file')) {
    ...
    function validate_post_file($file_name = "")
    {
        if (is_valid_file_to_upload($file_name)) {
            ...
            echo json_encode(array("success" => false, 'message' => 'Invalid File Type' . " ($file_name)"));
            exit();
        }
    }
...
if (!function_exists('move_temp_file')) {
    ...
    function move_temp_file($file_name, $target_path, $related_to = "", $source_path = NULL, $static_file_name = "")
    {
        ...
        if (!$source_path) {
            $source_path = "upload/temp/" . $file_name;
        }
...
{
    $images_extentions = array("jpg", "JPG", "jpeg", "JPEG", "png", "PNG", "gif", "GIF", "bmp", "BMP","mp4");
...
if (!function_exists('upload_file_to_temp')) {
    ...
    function upload_file_to_temp($target_path)
    {
        $ids = $_SESSION['filerandomno'];
        
        if (!empty($_FILES)) {
            $temp_file = $_FILES['file']['tmp_name'];
            $file_name = $ids.$_FILES['file']['name'];

            if (!is_valid_file_to_upload($file_name))
                return false;
             
            //$target_path = 'upload/testimg/';
            if (!is_dir($target_path)) {
                if (!mkdir($target_path, 0777, true)) {
                    die('Failed to create file folders.');
                }
...
if (!function_exists('is_valid_file_to_upload')) {
    ...
    function is_valid_file_to_upload($file_name = "")
    {
        ...
        $file_formates = explode('|', 'png|jpg|jpeg|gif|mp4|ogg|bmp');
