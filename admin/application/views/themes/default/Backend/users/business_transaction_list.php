    ...
    <div class="col-lg-12">
	  ...
	  <div class="card shadow mb-4">
	    <div class="card-header py-3">
		  ...
		  <h3 class="m-0 font-weight-bold text-primary"><?=my_caption('businesstrans_list_title')?></h3>
        </div>
        <div class="card-body">
		    ...
		    <table id="sort" class="table table-bordered exampletables">
			  <thead>
			    <tr>
			      ...
			      <th><?=my_caption('businesstrans_mode')?></th>
			      <!-- <th><?=my_caption('businesstrans_type')?></th>-->
			      <th><?=my_caption('followup_req')?></th>
				  <th style="width:20%!important;"><?=my_caption('followup_to_name')?></th>
				  <th style="width:20%!important;"><?=my_caption('followup_from_name')?></th>
			  ...
			  <tbody>
			      ...
			      ?>
			       <tr id="<?=$valdata->buddy_meet_id;?>">
			           ...
			           <td ><?=$valdata->bns_trans_mode;?></td>
			           <!-- <td ><?=$valdata->bns_trans_type;?></td> -->
			           <td ><?= ucwords($valdata->req_title);?></td>
			           <td ><?= ucwords($ci->getusername($valdata->bns_trans_touser))?></td>
			           <td ><?= ucwords($ci->getusername($valdata->bns_trans_byuser))?></td>
			           <td ><span style="float: right;"><i class='fas fa-rupee-sign fa-fw fa-1x'></i> <?= number_format($valdata->bns_trans_amount,2);?></span></td>
			           <td ><?=$valdata->bns_trans_date;?></td>
