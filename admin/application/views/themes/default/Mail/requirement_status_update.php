...
<?php
    $requirement = $this->db->where('id',$requirement_status->req_user_status_reqid)->get('requirements')->row();
    $user_info = $this->db->where('id',$requirement_status->req_user_status_addedby)->get('user')->row();
    $status_info = $this->db->where('req_status_id',$requirement_status->req_user_status_statusid)->get('requirements_status')->row();
?>
<table data-outer-table border="0" align="center" cellpadding="0" cellspacing="0" class="outer-table row" width="640" style="width:640px;max-width:640px;" data-module="blue-header">
  ...
  <tr>
    <td align="center" class="img-responsive container-padding">
        <img class="auto-width" style="display:block;width:100%;max-width:100%;border:0px;" data-image-edit data-url data-label="Header image" width="640" src="<?= base_url('assets/images/') ?>requirement status update.jpg" border="0" editable="true" alt="picture">
    </td>
  </tr>
  <!-- blue-header -->
</table>
