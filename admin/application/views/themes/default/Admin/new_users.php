  ...
  <div class="row">
    <div class="col-lg-8">
	  ...
	  <div class="card shadow mb-4">
        ...
        <div class="card-body">
		  <?php
		    echo form_open(base_url('admin/new_user_actions/'), ['method'=>'POST']);
		  ?>
		  ...
		  <div class="row form-group mb-4">	
			<div class="col-lg-4">
			  <label>Role</label>
			  <select class="form-control" name="rol_for">
			    <option value="0">Select Role</option>
			    <?php foreach($info_role as $valrole){?>      
			        <option value="<?=$valrole->ids?>" <?php if(!empty(set_value('rol_for'))) { if(set_value('rol_for') == $valrole->ids) { echo "selected"; }}   ?> ><?=str_replace('_', ' ',$valrole->name);?></option>
			    <?php }?>
			  </select>
			  <?php echo form_error('rol_for', '<small class="text-danger">', '</small>');?>
			</div>  
