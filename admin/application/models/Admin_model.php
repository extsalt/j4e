...
//		(my_post('enabled_approval') == '1') ? $enabled_approval = 1 : $enabled_approval = 0;
...
//		(my_post('html_purify') == '1') ? $html_purify = 1 : $html_purify = 0;
		...
		if(!empty($_FILES['infopic']['name']))
        {
            ...
            if($this->upload->do_upload('infopic'))
            {
                ...
                $type_img = $uploadData['file_name'];
                $update_data['cmp_logo'] = 'upload/cms/'.$type_img;
        ...
        if(!empty($_FILES['infopic1']['name']))
        {
            ...
            if($this->upload->do_upload('infopic1'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['login_page_logo'] = 'upload/cms/'.$type_img;
                $this->db->update('setting', $update_data);
        ...
        if(!empty($_FILES['userfile']['name']))
        {
            ...
            if($this->upload->do_upload('userfile'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['favicon'] = 'upload/cms/'.$type_img;
                $this->db->update('setting', $update_data);
        ...
        if(!empty($_FILES['userfile1']['name']))
        {
            ...
            if($this->upload->do_upload('userfile1'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['membership_image'] = 'upload/'.$type_img;
                $this->db->update('setting', $update_data);
...
//            }
		...
		$update_data = array(
		  'sys_name' => my_post('system_name'),
                    'support_no' => my_post('support_no'),
//		  'no_of_free_invite' => my_post('no_free_invite'),
...
//		  'invite_approval'=>$enabled_approval,
                    'razorpay_key'=>  my_post('razorpay_key'),
                    'razorpay_secret'=>  my_post('razorpay_secret'),
                    'app_version'=>  my_post('app_version'),
                    'app_version_title'=>  my_post('app_version_title')
		);
		$this->db->update('setting', $update_data);
		return TRUE;
        ...
        if(!empty($_FILES['userfile1']['name']))
        {
            ...
            if($this->upload->do_upload('userfile1'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['home_slider'] = 'upload/images/'.$type_img;
                $this->db->update('website_settings', $update_data);
//                echo $this->db->last_query();die;
...
//            }
        ...
        if(!empty($_FILES['userfile2']['name']))
        {
            ...
            $config['upload_path'] = 'upload/images/ads/';
            $config['allowed_types'] = 'jpg|jpeg|png';
            $config['file_name'] = $date_string.$ids;
            ...
            if($this->upload->do_upload('userfile2'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['advertise2'] = 'upload/images/ads/'.$type_img;
                $this->db->update('website_settings', $update_data);
//                echo $this->db->last_query();die;
...
//            }
        ...
        if(!empty($_FILES['userfile3']['name']))
        {
            ...
            if($this->upload->do_upload('userfile3'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['advertise3'] = 'upload/images/ads/'.$type_img;
                $this->db->update('website_settings', $update_data);
//                echo $this->db->last_query();die;
...
//            }
        ...
        if(!empty($_FILES['userfile4']['name']))
        {
            ...
            if($this->upload->do_upload('userfile4'))
            {
                $uploadData = $this->upload->data();
                $type_img = $uploadData['file_name'];
                $update_data['advertise4'] = 'upload/images/ads/'.$type_img;
                $this->db->update('website_settings', $update_data);
//                echo $this->db->last_query();die;
...
//            }
		...
		$update_data = array(
		  'home_slider_caption' => my_post('home_slider_caption'),
                    'support_mail' => my_post('support_mail'),
                    'android_link'=>  my_post('android_link'),
                    'ios_link'=>  my_post('ios_link'),
                    'fb_link'=>  my_post('fb_link'),
                    'in_link'=>  my_post('in_link'),
                    'yt_link'=>  my_post('yt_link'),
                    'map_link'=>  my_post('map_link'),
                    'address'=>  my_post('address'),
                    'about'=>  my_post('about'),
                    'highlighted_users'=>  implode(',', my_post('highlighted_users'))
		);
		$this->db->update('website_settings', $update_data);
		return TRUE;
	...
	public function invite_user() {
		...
		if ($query->num_rows()) {
			...
			$email = array(
			  'email_to' => my_post('email_address'),
	...
	public function payment_setting_update() {
		$payment_setting = array(
		  'type' => my_post('payment_type'),
		  'feature' => my_post('payment_feature'),
		  'tax_rate' => my_post('payment_tax_rate'),
		  'stripe_one_time_enabled' => my_post('stripe_one_time_enabled'),
		  'stripe_recurring_enabled' => my_post('stripe_recurring_enabled'),
		  'stripe_publishable_key' => my_post('stripe_publishable_key'),
		  'stripe_secret_key' => my_post('stripe_secret_key'),
		  'stripe_signing_secret' => my_post('stripe_signing_secret'),
		  'paypal_one_time_enabled' => my_post('paypal_one_time_enabled'),
		  'paypal_recurring_enabled' => 0,
		  'paypal_client_id' => my_post('paypal_client_id'),
		  'paypal_secret' => my_post('paypal_secret'),
		  'paypal_webhook_id' => my_post('paypal_webhook_id')
		);
