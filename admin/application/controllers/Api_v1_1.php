	...
	public function app_user_verify_otp_post(){
	    ...
	    if (isset($this->json_array['otp']) && isset($this->json_array['userid']) && isset($this->json_array['mobile']) ) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
				...
				$rs = $this->user_model->get_app_user_verify_otp($this->json_array['otp'],$this->json_array['userid'],$this->json_array['mobile'],$this->json_array['device_type'],$this->json_array['device_token'],$this->setting);
				if ($rs['status'] != FALSE) {
					//$this->user_model->sigin_success_log($rs, 'api');
					//$resp_array = array('status' => TRUE);
	...
	public function app_user_account_details_post(){
	    ...
	    if (isset($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
				...
				$rs = $this->user_model->get_app_user_account_details($this->json_array['userid']);
				if ($rs != FALSE) {
					//$this->user_model->sigin_success_log($rs, 'api');
					//$resp_array = array('status' => TRUE);
		...
		if (isset($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($rschk==FALSE){
			        ...
			        $resp_array = array(
        			  'status' => FALSE,
        			  'msg' => 'No Connection requestion sent',
        			  'data'=>$obj
        			);
		...
		if (isset($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($rschk==FALSE){
			        ...
			        $resp_array = array(
        			  'status' => FALSE,
        			  'msg' => 'No Connection requestion receive',
				      'data'=>$obj
        			);
		...
		if (isset($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($rschk==FALSE){
			        ...
			        $resp_array = array(
					  'status' => TRUE,
					  'msg' => 'No connection sent request  found',
					  'data'=>$obj
					);
	    ...
	    if (isset($this->json_array['userid']) && !empty($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($check_req_title==FALSE){
                  	    $resp_array = array(
					       'status' => FALSE,
    					  'msg' => 'No Requirments Created Till now',
				  'data'=>$obj
    					);
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			  ...
			  'data'=>$obj
			    ...
			    if($check_req_title==FALSE){
                  	    $resp_array = array(
					       'status' => FALSE,
    					  'msg' => 'No Pending Requirments Till now',
			              'data'=>$obj
    					);
	    ...
	    if (isset($this->json_array['userid']) && !empty($this->json_array['userid'])) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($check_req_title==FALSE){
                  	    $resp_array = array(
					       'status' => FALSE,
    					  'msg' => 'No Complete Requirments Till now',
				          'data'=>$obj
    					);
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($check_req_title==FALSE){
			        ...
			        if($get_all_leads==FALSE){
			            ...
			            $resp_array = array(
					       'status' => TRUE,
    					  'message' => 'Follow are the list of leads',
    ...
    public function app_get_requirement_details_post(){
	    ...
	    if (isset($this->json_array['requirement_id']) && !empty($this->json_array['requirement_id']) ) {
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    //check requirement title exist or not
			    $check_requirement_exist=$this->user_model->get_all_requirement_details($this->json_array['requirement_id']);
			    if($check_requirement_exist==FALSE){
			         ...
			         $resp_array = array(
					       'status' => TRUE,
    					  'message' => 'Following are the requirement details are',
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			   ...
			   // $check_recomended_exist=$this->user_model->get_all_recomendate_ee_old($this->json_array['userid'],$this->json_array['requirement_id']);
			    if($check_recomended_exist==FALSE){
			         ...
			         $resp_array = array(
					       'status' => TRUE,
    					  'message' => 'Following are the recomendation list  details are',
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($checkuserexist['status']==FALSE){
			        ...
			        if(intval($this->json_array['ratings'])>5){
			             ...
			             $resp_array = array(
					       'status' => FALSE,
    					   'msg' => "Ratings will be accept only value between 1-5",
				           'data' => $obj
    					);
			        ...
			        if($ratings_review_exist['status']==FALSE){
			            ...
			            if(isset($user_rating_rev) && !empty($user_rating_rev)){
			                ...
			                $get_userratings=$this->user_model->get_user_ratings_data($user_rating_rev);
    			             ...
    			             $resp_array = array(
    					       'status' => FALSE,
        					   'msg' => 'Some problem occurred on user ratings and review inserting',
				               'data' => $obj
        					);
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($checkuserexist['status']==FALSE){
			       ...
			       if($viewgallery['status']==FALSE){
			           ...
			           $gallery=$this->user_model->view_profile_gallery_field_builder($viewgallery['data']);
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
				...
				if ($rs != FALSE) {
				   ...
				   else
				   {
					...
					$data_array = $this->user_model->get_app_get_lead_status_fileds_builder($rs);
	    ...
	    isset($this->json_array['created_date']) && !empty($this->json_array['created_date']) && isset($this->json_array['created_time']) && !empty($this->json_array['created_time']) ) { // isset($_FILES) && !empty($_FILES) &&  //&& isset($this->json_array['address']) && !empty($this->json_array['address'])
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if ($_FILES['post_thumbnail']['name'] != '') {
            				...
            				//GET USER ID
            				$avatar_file = $useriddd[0]->ids.'.'.pathinfo($_FILES['post_thumbnail']['name'], PATHINFO_EXTENSION);
...
{   
   ...
   public function app_user_book_event_post(){
	    ...
	    && isset($this->json_array['userphno']) && !empty($this->json_array['userphno']) && isset($this->json_array['amount']) && !empty($this->json_array['amount']) &&
	    ...
	    { 
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			    ...
			    if($check_req_title==FALSE){
			        ...
			        $insert_participate= $this->user_model->save_participation_status_deetails($this->json_array);
			            ...
			            else
			            {
			                 $resp_array = array(
    					       'status' => TRUE,
        					   'msg' => 'Participation Successfully',
			    ...
			    else
			    {
			         $resp_array = array(
					      'status' => FALSE,
    					  'msg' => 'Once You Participation for this event',
    					  'data'=>$check_req_title
    					);
			...
			if (!$throttle_check['result'] && $this->throttle_switch) {
			   ...
			   $check_invite_user = $this->user_model->check_guest_event_invite($this->json_array['mobilenumber'],$this->json_array['eventid']);
			    ...
			    else
			    {
			        $resp_array = array(
    					       'status' => FALSE,
        					   'msg' => 'Once Guest Invited Send for this Event',
        					   'data' =>$obj
        					);
   ...
   public function app_get_my_diary_post(){
	    ...
	    $bns_trans_obj = $testnml_obj = $myaward_obj = $myrecogn_obj = $memrbprofit_obj = $recommendedto_obj = $recommendedby_obj = $myreport_obj = $guestinvt_obj = array();
	    ...
	    { 
			...
			else
			{
			 ...
			 //sprintf("%02d", $get_fallow_up)
			  ...
			  "Award"=>$myaward_obj,"Recognitions"=>$myrecogn_obj,"MembershipBenefits"=>$memrbprofit_obj,"RecommendedTo"=>$recommendedto_obj,"RecommendedBy"=>$recommendedby_obj,
